...
(function() {
  ...
  console.log('[VBH Widget v2.2] Shadow DOM Web Component loading...');
  ...
  var WIDGET_STYLES = `
    ...
    * {
    ...
    .vbh-header {
      background: linear-gradient(135deg, #1e3a5f 0%, rgba(30, 58, 95, 0.9) 100%);
      color: white;
      padding: 16px;
      display: flex;
      align-items: center;
  ...
  class VBHChatbot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      console.log('[VBH Widget v2.2] Initialized successfully');
    }
    ...
    getMessageHTML(msg, index) {
      ...
      if (!isUser && msg.citations && msg.citations.length > 0) {
        ...
        html += '<div class="vbh-citations-label">Here\'s how I found this answer</div>';
        html += '<div class="vbh-citation-card">';
        html += '<a href="' + this.escapeHtml(citation.url) + '" target="_blank" rel="noopener noreferrer" class="vbh-citation-title">';
        html += this.escapeHtml(citation.title || 'Reference');
        html += '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6M15 3h6v6M10 14L21 3"/></svg>';
        html += '</a>';
        
        if (citation.description) {
          html += '<p class="vbh-citation-description">' + this.escapeHtml(citation.description) + '</p>';
        } else {
          html += '<p class="vbh-citation-description" style="opacity:0;">placeholder</p>';
        }
